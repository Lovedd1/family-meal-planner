...
- [x] 健康页 - 近7天饮食记录详情弹窗（点击记录查看早中晚完整菜单）
- [x] 健康页 - 30天体重趋势柱状图（CSS实现，可左右滑动，涨红跌绿颜色区分，进度条根据减肥/增肥/维持目标动态计算）
- [x] Bug修复 - 重置所有数据后体重趋势柱状图未清空问题（drawWeightChart空数据时清空数组 + 设置页重置后自动返回健康页刷新）
