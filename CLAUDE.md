...
- [x] 点餐页 - 添加菜品时底部弹出餐次选择
- [x] 今日菜单页 - 新增营养Tab（碳水/蛋白质/脂肪/膳食纤维统计）
- [x] 今日菜单页 - 菜品卡片显示营养标签
...
- [x] Bug修复 - 健康页一日分析弹窗文本溢出右边框问题
- [x] Bug修复 - 生理期日历高亮功能（经期红色、易孕期黄色、今天绿色）、添加防御性检查修复计算问题
- [x] 重设计 - 生理期追踪日历（分段时间线 + 星期标题 + 实际日期显示）
- [x] style - 调整阶段时间线与图例间距（margin-bottom: 32rpx）
...
- [x] 双人同步实时更新（今日菜单3秒、冰箱和自定义菜品5秒、做饭记录8秒轮询 + 页面自动刷新）
- [x] 点餐页面精简为4道菜（碳水：米饭、蛋白质：土豆炖牛肉、脂肪：凉拌黄瓜、膳食纤维：西兰花炒鸡胸肉）
- [x] 冰箱页面新增水果分类（🍎）
- [x] 确认菜单时调用DeepSeek API实时评估食材风险，在确认弹窗顶部显示风险提示
- [x] 做饭记录页面（拍照归档、关联菜品、评分、统计）
- [x] 做饭记录页面重设计（照片墙瀑布流、全屏详情页/编辑页、1-10评分选择器）
